...
function dis_dir($conn){
	...
	for($i=0;$i<count($res);$i++){
		if($res[$i]['Depth']==0){
			echo "<div class='dir0' id='dbdir".$res[$i]['Id']."'"."><p onclick='toggle_children(this)'>".$res[$i]['Name'].'</p>';
			for($j=0;$j<count($res);$j++){
				if($res[$j]['ParentId']==$res[$i]['Id']){
					echo "<div class='dir1' id='dbdir".$res[$j]['Id']."'"."><p onclick='toggle_children(this)'>".$res[$j]['Name'].'</p>';
					for($k=0;$k<count($res);$k++){
						if($res[$k]['ParentId']==$res[$j]['Id']){
							echo "<div calss='dir2' id='dbdir".$res[$k]['Id']."'"."><p onclick='toggle_children(this)'>".$res[$k]['Name'].'</p>';
							echo '</div>';
						}
					}
					echo '</div>';
